...
impl Index {
    ...
    pub fn from_registry(registry: &Registry, frecency: Frecency) -> Index {
        let entries = registry
            ...
            .collect();
        Index { entries, frecency }
    }

    /// Merges a target's `list_actions` handshake response into the
    /// catalog: an id the registry already knows gets its title/native
    /// command/source updated in place (its `key_hint`, which the target
    /// has no way to know, is left untouched); an id the target reports
    /// that isn't already present is added outright.
    pub fn merge_remote(&mut self, source: &str, remote: Vec<RemoteAction>) {
        for action in remote {
            match self.entries.iter_mut().find(|e| e.action_id == action.id) {
                Some(existing) => {

...
mod tests {
    ...
    /// A hand-built `Registry` matching the two local actions the tests
    /// below assume (`close.tab` bound to `ctrl+w`, `save` unbound) -- there
    /// is no config-file format left to declare these from (see
    /// `src/config/mod.rs`'s doc comment), so this bypasses `Registry::load`
    /// entirely rather than relying on it for a still-empty action table.
    fn registry() -> Registry {
        Registry::with_actions_and_triggers(
            ...
            std::collections::HashMap::from([(
                crate::config::KeyCombo::parse("ctrl+w").unwrap(),
                "close.tab".to_string(),
            )]),
        )
    }
